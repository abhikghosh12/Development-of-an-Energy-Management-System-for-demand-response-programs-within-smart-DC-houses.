...
within ;
model HPController_DR_AC_PV_SC "HPController_DR_AC_PV_SC.mo"
	...
	parameter String HPFile=GreenBuilding.Utilities.Functions.getModelDataDirectory()+"\\heat_pump\\hp_data\\Stiebel_Eltron\\WPL_10_ACS\\SE_WPL_10_ACS2.txt" "File name for heat pump characteristics" annotation(Dialog(
		group="Heating Power",
		tab="HP - Power Data"));
	Boolean state "PV power  based model state on/off" annotation(Dialog(
		group="I/O",
		tab="Results 1",
	...
	public
		...
		input Modelica.Blocks.Interfaces.RealInput TFlowRef(
			...
			Dialog(
				...
				tab="Results 1",
				showAs=ShowAs.Result));
		input Modelica.Blocks.Interfaces.RealInput EPriceAverage(quantity="price") "price" annotation(
			Placement(

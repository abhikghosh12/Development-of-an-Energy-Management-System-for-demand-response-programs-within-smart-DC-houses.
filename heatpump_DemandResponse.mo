...
model HeatPump_demandResponse "heatpump_DemandResponse.mo"
	protected
	...
	public
		...
		Modelica.Blocks.Tables.CombiTable1D PelSP(
			...
			Placement(transformation(extent={{-15,30},{5,50}})),
			Dialog(
				group="Coefficient of Performance",
				tab="Results 1"));
	...
	public
		...
		GreenBuilding.Interfaces.Electrical.DefineCurrentAC1 COMPGridConnection1 if CompPhase==1 annotation(
			...
			Dialog(
				group="Timing",
				tab="Results 2"));
		parameter Real EPriceAvg=48 "Average price of grid";
	...
	equation
			  EPricing.u[1] = AmbientConditions.HourOfDay;
					...
					if AuxHeat then
						when (AUXon and not (time-StartTimeAux)<tAUX and StartTimeAux>time.start) then

...
within ;
model BatteryCharger_DR "BatteryCharger_DR_variable tariff.mo"
	...
	Boolean Price "pRICING FUNCTION on/off-switch";
	Modelica.Blocks.Tables.CombiTable1D EPricing(
		tableOnFile=true,
		tableName=PriceTable,
		fileName=HPFile) "Pricing function of grid" annotation(
		Placement(transformation(extent={{25,5},{45,25}})),
		Dialog(
	...
	parameter String HPFile=GreenBuilding.Utilities.Functions.getModelDataDirectory()+"\\heat_pump\\hp_data\\Stiebel_Eltron\\WPL_10_ACS\\SE_WPL_10_ACS2.txt" "File name for heat pump characteristics" annotation(Dialog(
		group="Heating Power",
		tab="HP - Power Data"));
	GreenBuilding.Interfaces.Electrical.ThreePhase Grid3 if ChargePhase==3 "3-phase connection to Grid" annotation(Placement(
		transformation(extent={{170,10},{190,30}}),
		iconTransformation(extent={{190,90},{210,110}})));
	GreenBuilding.Interfaces.Electrical.SinglePhase Grid1 if ChargePhase==1 "1-phase connection to Grid" annotation(Placement(
		transformation(extent={{170,-75},{190,-55}}),
	...
	public
		...
		parameter Real SOCMin(
			...
			group="State of Charge",
			tab="Parameters"));
		GreenBuilding.Utilities.Electrical.B6Converter B6Converter(eta=eta) if ChargePhase==3 "B6-Converter for 3-phase-grid-connection" annotation(
			Placement(transformation(extent={{-10,-10},{10,10}})),
			Dialog(
				group="State of Charge",
				tab="Parameters"));
		GreenBuilding.Utilities.Electrical.B6CControl B6CController(
			TPI=TPI,
			kPI=kPI,
			PMax=PMax) if ChargePhase==3 "B6C-Controller for 3-phase-grid-connection" annotation(
			Placement(transformation(extent={{-10,-10},{10,10}})),
			Dialog(
				group="State of Charge",
				tab="Parameters"));
		...
		GreenBuilding.Utilities.Electrical.B2CControl B2CController(
			...
			Dialog(
				group="State of Charge",
				tab="Parameters"));
		input Modelica.Blocks.Interfaces.RealInput EPriceAverage(quantity="price") "price" annotation(Placement(
			transformation(extent={{-20,-20},{20,20}}),
			iconTransformation(extent={{-220,130},{-180,170}})));
		parameter String PriceTable="Pricing" "Table name for Eprice " annotation(Dialog(
			group="Heating",
			tab="HP - Power Data"));
	algorithm
	...
	initial equation
		assert (ChargePhase==1 or ChargePhase==3, "ChargePhase must equal 1 or 3");
		if (ChargePhase==1) then	
			assert (ControlBus.Voc<=0.9*Grid1.Veff1, "DC-voltage too high for B2-Converter");
		end if;
		if (ChargePhase==3) then
			assert (ControlBus.Voc<=2.34*Grid3.Veff3[1], "DC-voltage too high for B6-Converter");
		end if;
	equation
		EPricing.u[1] = AmbientConditions.HourOfDay;
			                       if ( EPriceAverage > EPricing.y[1] ) then
						               Price = true;	
						else 
								Price=false;
							end if;
			...
			B6CController.Voc=ControlBus.Voc;
			B6CController.VIC=ControlBus.VIC;
			B6CController.VICMax=ControlBus.VICMax;
			B6CController.VICMin=ControlBus.VICMin;
			connect(B6Converter.alphaRef,B6CController.alphaRef) annotation(Line(points=0));
			connect(Grid3,B6Converter.Grid3) annotation(Line(points=0));
			connect(DC,B6Converter.DC) annotation(Line(points=0));
			connect(B6Converter.VGrid,B6CController.VGrid) annotation(Line(points=0));
			connect(B6Converter.IDC,B6CController.IDC) annotation(Line(points=0));
			if (((discharged and PRef>Pth) or (charged and PRef<-Pth)) and (PRef>Pth or PRef<-Pth)) then
				B6CController.PRef=PRef;
			else
				B6CController.PRef=0;
			end if;
			VICAct=B6Converter.VIC;
			IDCAct=B6Converter.IDC;
			PDC=DC.I*DC.V;
			...
			connect(B2Converter.alphaRef,B2CController.alphaRef) annotation(Line(points=0));
			connect(Grid1,B2Converter.Grid1) annotation(Line(points=0));
			connect(DC,B2Converter.DC) annotation(Line(points=0));
			connect(B2Converter.VGrid,B2CController.VGrid) annotation(Line(points=0));
			connect(B2Converter.IDC,B2CController.IDC) annotation(Line(points=0));
			if (((discharged and PRef>Pth) or (charged and PRef<-Pth)) and (PRef>Pth or PRef<-Pth)) then
				B2CController.PRef=PRef;
			elseif ( Price and (ControlBus.SOC < SOCMin + 0.5 )) then
			  
					B2CController.PRef= 5000;
			...
			PDC=DC.I*DC.V;
			PAC=Grid1.Veff1*Grid1.Ieff1*cos(Grid1.PhiPhase1);
			QAC=Grid1.Veff1*Grid1.Ieff1*sin(Grid1.PhiPhase1);			
			end if;
	annotation(
		Icon(

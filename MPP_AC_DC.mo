...
within ;
model MPP_AC_DC "Maximum power point tracker customised for AC and DC system"
	GreenBuilding.Interfaces.Electrical.ThreePhase Grid3 if PVPhase==3 "3-phase connection to Grid" annotation(Placement(
		transformation(extent={{240,-5},{260,15}}),
		iconTransformation(extent={{190,90},{210,110}})));
	GreenBuilding.Interfaces.Electrical.SinglePhase Grid1 if PVPhase==1 "1-phase connection to Grid" annotation(Placement(
	...
	public
		...
		parameter Integer PVPhase=1 "Number of phases PVConverter is connected to" annotation(Dialog(
			...
			tab="Parameters"));
	initial equation
		assert(PVPhase==3 or PVPhase==1,"PVPhase must equal 1 or 3");
	equation
			if PVPhase==1 then
			B2Converter.Connected=true;
			B2Converter.alphaRef=acos(min(max(MPP/(0.9*B2Converter.VGrid),-1),1));		
		connect(B2Converter.DC,PVDC) annotation(Line(points=0));
		connect(B2Converter.Grid1,Grid1) annotation(Line(points=0));
		PDC=PVDC.I*PVDC.V;
		...
		connect(B6Converter.DC,PVDC) annotation(Line(points=0));
		connect(B6Converter.Grid3,Grid3) annotation(Line(points=0));
		PDC=PVDC.I*PVDC.V;
		...
		GridConnection.V = 48;
							if (GridConnection.V>1e-10) then
								GridConnection.I=(PDC*alpha)/GridConnection.V;
								
							else
								GridConnection.I=0;
								
							end if;
							PDCoutput=GridConnection.I*GridConnection.V;
							PACnet =PAC - PDCoutput;
